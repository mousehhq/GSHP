...
<script>
    ...
    export default {
            ...
            return {
                ...
                rules: {
                    ...
                    hotYear:[
                        ...
                        {validator: validatehotYearRepe, trigger:'blur'}
                    ],
                    coldYear:[
        ...
        methods: {
            ...
            onSubmit(formName) {
                var self = this;
                console.log("self.$refs.hotupload.uploadFiles",self.$refs.hotupload.uploadFiles);
                self.$refs[formName].validate((isPass) => {
                    console.log("validate",formName);
                    if (isPass) {
            ...
            allProjectsSuccess(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var item = data[i];
                    if (self.$route.query.id && item.id.toString() === self.$route.query.id.toString()) {
                        activeIndex = i;
                    }
                    var obj = {
                        id: item.id,
                ...
                this.$emit('totalCnt', this.allprojectLists.length);
                var evaluateCurrentPage = Math.floor(activeIndex / this.pageSize) + 1;
                this.handleCurrentChange(evaluateCurrentPage ? evaluateCurrentPage : 1);
            },
