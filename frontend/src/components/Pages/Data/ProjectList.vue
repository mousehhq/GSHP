            ...
            <el-dialog :title="isEdit?'上传参数表格':'新增项目'" :visible.sync="addProjectVisible" width="1000px">
                <el-form :model="form" :rules="rules" ref="addForm" :inline="true">
                    ...
                    <el-row>
                        ...
                        <el-col :span="10" class="attention-info el-icon-info upLoadTip" v-if="hotTip">
                            请上传<span>{{form.hotYear}}</span>年供暖季<span>{{form.hotYear}}</span>/4/15-<span>{{form.hotYear}}</span>/11/15期间内的数据
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="8">
                        ...
                        <el-col :span="10" class="attention-info el-icon-info upLoadTip" v-if="coldTip">
                            请上传<span>{{form.coldYear}}</span>年供冷季<span>{{form.coldYear}}</span>/11/15-<span>{{parseInt(form.coldYear) + 1}}</span>/4/15期间内的数据
                        </el-col>
                    </el-row>
                </el-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            addItem(model) {
                var self = this;
                self.loading = self.$loading.service({
                    lock: true,
                    text: '拼命加载中',
...
//                        self.getAllProjects();
            ...
            checkHasFile(model){
                ...
                let confirmStr = '您已经上传过该项目';
                let hasHot = self.form.hotYear && self.$refs.hotupload.uploadFiles && self.$refs.hotupload.uploadFiles.length > 0 && self.allHotDataList.length > 0;
                let hasCold = self.form.coldYear && self.$refs.coldupload.uploadFiles && self.$refs.coldupload.uploadFiles.length > 0 && self.allColdDataList.length > 0;
                if (hasHot) {
                    confirmStr = confirmStr + self.form.hotYear + '年暖季';
                }
                if (hasCold) {
                    if (hasHot) {
                        confirmStr = confirmStr + '、';
                    }
                    confirmStr = confirmStr + self.form.coldYear + '年冷季';
                }
                confirmStr = confirmStr + '的表格，此操作将覆盖已经上传的表格, 是否继续?';
            ...
            editItem(model) {
                ...
                self.$http.post(url, model).then(function (response) {
                    ...
                    if (response.status === 200) {
                        self.addProjectVisible = false;
                        let backYear = null;
                        if (self.form.hotYear) {
                            backYear = self.form.hotYear;
                        } else if (self.form.coldYear) {
                            backYear = self.form.coldYear;
                        }
                        self.getAllProjects();
                        self.$emit('editSuccess', {name: self.form.name, year: backYear});//这块可以让后台直接传回上传表格的数据，不用再发一次请求获取
            ...
            hotYearChange() {
                var self = this;
                if (this.$refs.hotupload.uploadFiles && this.$refs.hotupload.uploadFiles.length > 0) {
                    this.hotTip = false;
                } else {
                    this.hotTip = (this.form.hotYear != null) ? true : false;
                }
                let url = GSHP_SERVER_ENDPOINT + '/dataProject/getParamsByIdAndYearAndSeaon';
                self.$http.get(url, {
                    params: {
                        id: self.$route.query.id,
                        year: self.form.hotYear,
                        isHot: true,
                    }
                }).then((response) => {
                    if (response.status === 200) {
                        this.allHotDataList = response.data;
                    }
                });
            },
            ...
            coldYearChange() {
                var self = this;
                if (this.$refs.coldupload.uploadFiles && this.$refs.coldupload.uploadFiles.length > 0) {
                    this.coldTip = false;
                } else {
                    this.coldTip = (this.form.coldYear != null) ? true : false;
                }
                let url = GSHP_SERVER_ENDPOINT + '/dataProject/getParamsByIdAndYearAndSeaon';
                self.$http.get(url, {
                    params: {
                        id: self.$route.query.id,
                        year: self.form.coldYear,
                        isHot: false,
                    }
                }).then((response) => {
                    if (response.status === 200) {
                        this.allColdDataList = response.data;
                    }
                });
            },
            ...
            onHotChange(file, fileList) {
                ...
                //fileList = this.form.hotFile;
                if (this.form.hotFile) {
                    this.hotTip = false;
                }
            },
